...
//	Last angle value, in °, calculated from the accelerometer and the gyroscope,
	...
	private double filteredAngleZ = 180.;
	...
	if(filteredAngleZ>360) {
		filteredAngleZ=0;
		filteredAngleZ += Math.round(deltaGyroAngleZ*100)/100.;
	}else if(filteredAngleZ<0){
		filteredAngleZ= 360;
		filteredAngleZ += Math.round(deltaGyroAngleZ*100)/100.;
	}else filteredAngleZ = filteredAngleZ + Math.round(deltaGyroAngleZ*100)/100.;
